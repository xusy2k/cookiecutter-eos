...
{{ cookiecutter.default_contract }} demonstrates the eosio platform running a blockchain as a local single node test net with a simple DApp, {{ cookiecutter.default_contract }}. {{ cookiecutter.default_contract }} allows users to create and update notes. This guide uses scripts, containing relevant commands, which will show you how to install, build and run {{ cookiecutter.default_contract }}, and by doing so will demonstrate:
...
The sample DApp demonstrates storing data in multi index table and retrieving this data into the web based UI. {{ cookiecutter.default_contract }} is a simple note taking application, where notes are tied to user accounts. For this example, all accounts are pre-created by scripts and the account details are displayed at the bottom of the {{ cookiecutter.default_contract }} UI.
...
```sh
docker stop {{ cookiecutter.eosio_container }}
```

# Detailed guide

In this section we will describe in detail each script used to run the {{ cookiecutter.default_contract }} environment in details.
...
```js
{{ cookiecutter.default_contract }} // project directory
├── eosio_docker
│   ├── * contracts // this folder will be mounted into docker
│   │   └── {{ cookiecutter.default_contract }}
│   │       └── {{ cookiecutter.default_contract }}.cpp // the main smart contract
...
The UI, index.jsx, reads the notes data directly from nodeos using 'getTableRows()'. The smart contract, {{ cookiecutter.default_contract }}.cpp, stores these notes in the multi index table using 'emplace()'' and 'modify()'.

## Docker usage

Docker is used to wrap the eosio software inside and run a container (instance) from an image (eosio/eos-dev {{ cookiecutter.eos_dev_tag }}). To work with the blockchain directly, by running the scripts or using a cleos command line, you need to go into the container bash.

Go into container bash:

```sh
docker exec -it {{ cookiecutter.eosio_container }} bash
```
...
The smart contract can be found at `eosio_docker/contracts/{{ cookiecutter.default_contract }}/{{ cookiecutter.default_contract }}.cpp`, you can edit this smart contract. You will then need to compile and deploy the contract to the blockchain.
...
```sh
./scripts/deploy_contract.sh {{ cookiecutter.default_contract }} {{ cookiecutter.default_account }} {{ cookiecutter.default_contract }}wal $(cat {{ cookiecutter.default_contract }}_wallet_password.txt)
```

After running this script the modified smart contract will be deployed on the blockchain.

Remember to redeploy the {{ cookiecutter.default_contract }} contract each time you modify it using the steps above!

## Frontend:

The UI code can be found at {{ cookiecutter.default_contract }}/frontend/src/pages/index.jsx), once you have edited this code the frontend react app compile automatically and the page on browser will be automatically refreshed. You can see the change on the browser once the browser finishes loading.
...
```sh
docker run --rm --name {{ cookiecutter.eosio_container }} \
-p 8888:8888 -p 9876:9876 \
--mount type=bind,src="$(pwd)"/contracts,dst=/opt/eosio/bin/contracts \
--mount type=bind,src="$(pwd)"/scripts,dst=/opt/eosio/bin/scripts \
--mount type=bind,src="$(pwd)"/data,dst=/mnt/dev/data \
-w "/opt/eosio/bin/" eosio/eos-dev:{{ cookiecutter.eos_dev_tag }} /bin/bash -c "./scripts/init_blockchain.sh"
```
...
```sh
docker rm -f {{ cookiecutter.eosio_container }}
```
