...
# download eosio/eos-dev:{{ cookiecutter.eos_dev_tag }} image
echo "=== pull eosio/eos-dev image {{ cookiecutter.eos_dev_tag }} from docker hub ==="
docker pull eosio/eos-dev:{{ cookiecutter.eos_dev_tag }}
...
docker stop {{ cookiecutter.eosio_container }} || true && docker rm --force {{ cookiecutter.eosio_container }} || true
